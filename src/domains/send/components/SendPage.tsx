...
import SendProgressModal from '@/domains/send/components/SendProgressModal'
import { FileUploadSection } from '@/domains/send/components/send-page/FileUploadSection'
import { ManualCreateSection } from '@/domains/send/components/send-page/ManualCreateSection'
import { DataTableSection } from '@/domains/send/components/send-page/DataTableSection'
import { AttachmentsSection } from '@/domains/send/components/send-page/AttachmentsSection'
// WhatsApp button functions are tracked inline
import { MessageSection } from '@/domains/send/components/send-page/MessageSection'
import { ContactChannelSection } from '@/domains/send/components/send-page/ContactChannelSection'
...
export default function SendPage({ onNavigate }: SendPageProps) {
  ...
  const [whatsappTemplatePreview, setWhatsappTemplatePreview] = useState<WhatsAppTemplatePreview | null>(null)
  ...
  const whatsappTemplateButtons = React.useMemo(
  ...
  useEffect(() => {
    setWhatsappButtonActions(prev => {
      const next: Record<string, 'none' | 'attachment'> = {}
      whatsappTemplateButtons.forEach(button => {
        next[button.payload] = prev[button.payload] ?? 'attachment'
      })
      return next
    })
  ...
  async function handleSend() {
    ...
    if (channel === 'whatsapp') {
      ...
      const attachmentButtons = whatsappTemplateButtons.filter(
        button => (whatsappButtonActions[button.payload] ?? 'attachment') === 'attachment'
      )

      if (attachmentButtons.length > 0 && attachments.length === 0) {
        setErrorModal({
          title: 'Nenhum arquivo adicionado',
          message: 'Adicione pelo menos um arquivo para os botões com função Anexo.'
        })
        return
      }
    ...
    if (fileColumn) {
      ...
      filteredRows.forEach((row, index) => {
        ...
        if (matchedFiles.length === 0) {
          ...
          matchedFiles.forEach(f => {
            filesReferencedSet.add(normalizeAttachmentReference(f.name))
      ...
      unusedFiles = attachments
        .map(f => f.name)
        .filter(name => !filesReferencedSet.has(normalizeAttachmentReference(name)))
  ...
  async function sendMessages(rowsOverride?: Row[]) {
    ...
    if (channel === 'email') {
      const payload: any = {
        ...
        message,
        subject,
        email_sender: selectedEmailSender,
        app_password: selectedEmailSenderRecord?.appPassword || '',
        email_template_title: selectedEmailTemplateTitle || null
      }
    ...
    if (channel === 'whatsapp') {
      const templateParams = whatsappTemplateVariables.map(variable => {
      ...
      const buttonAttachmentBindings = attachmentButtons.map(button => ({
        button_payload: button.payload,
        file_column: fileColumn || null,
        match_mode: fileColumn ? matchMode : null
      }))

      const whatsappPayloadObj: Record<string, unknown> = {
        channel,
        mode: attachmentButtons.length > 0 ? 'template_with_pending_attachments' : 'template',
        whatsapp_sender_id: effectiveWhatsappSender?.id || '',
        whatsapp_template_title: selectedWhatsappTemplateTitle,
        whatsapp_template_language_code: whatsappTemplatePreview?.language || 'pt_BR',
        contact_column: contactColumn,
        rows: rowsForDispatch,
        whatsapp_template_variables: templateParams,
        idempotency_key: createIdempotencyKey()
      }

      if (buttonAttachmentBindings.length > 0) {
  ...
  return (
    <div>
      ...
      {channel === 'whatsapp' && selectedWhatsappTemplateTitle && whatsappTemplateButtons.length > 0 && (
        <div className={`mb-4 p-4 rounded ${currentTheme.bg} border-2 ${currentTheme.border}`}>
          <h3 className="font-medium mb-2">Função dos botões</h3>
          <div className="space-y-2">
            {whatsappTemplateButtons.map(button => (
              <div key={button.payload} className="flex items-center gap-3">
                <span className="text-sm flex-1">{button.label}</span>
                <select
                  value={whatsappButtonActions[button.payload] ?? 'attachment'}
                  onChange={e => setWhatsappButtonActions(prev => ({ ...prev, [button.payload]: e.target.value as 'none' | 'attachment' }))}
      ...
      {channel === 'whatsapp' && selectedWhatsappTemplateTitle && whatsappTemplateButtons.some(b => (whatsappButtonActions[b.payload] ?? 'attachment') === 'attachment') && (
        <AttachmentsSection
          attachments={attachments}
          headers={headers}
          fileColumn={fileColumn}
